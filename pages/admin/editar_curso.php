...
function selected_course()
{
        ...
        $select_course_id = "SELECT * FROM cursos WHERE id = '$id'";

        $query_exec = $mysqli->query($select_course_id);
        $course_id = $query_exec->fetch_assoc();
...
function edit_course()
{
        ...
        if (isset($_POST['enviar'])) {
                ...
                $preco = $mysqli->real_escape_string($_POST['preco-curso']);
                ...
                if (count($erro) > 0) {
                        ...
                        if (isset($_FILES['imagem']) && isset($_FILES['imagem']['size']) && isset($_FILES['imegem']['size']) > 0) {
                        ...
                        if ($upload !== false) {
                                ...
                                if ($imagemAlterada) {
                                        $sql_query = "UPDATE CURSOS SET titulo = '$title' , descricao = '$desc_smart', conteudo = '$desc', data_cadastro = NOW(), preco = '$preco', imagem = '$upload' WHERE id = '$id'";
                                } else {
                                        $sql_query = "UPDATE CURSOS SET titulo = '$title' , descricao = '$desc_smart', conteudo = '$desc', data_cadastro = NOW(), preco = '$preco' WHERE id = '$id'";
                                }
                                ...
                                if ($sql_exec) {
                                        $sucess = "Curso editado com sucesso! <a class='color_primary' href='index.php?p=gerenciar_cursos&admin=true'> Retornar a página de cursos </a>";
                                        return [
                                                "status" => 200,
                                                "success" => $sucess
                                        ];
        ...
        <div class="row align-items-end">
                <div class="col-lg-8">
                        <div class="page-header-title">
                                ...
                                <div class="d-inline">
                                        <h4>Edite as informações do seu curso</h4>
                                        <span>Realize as edições no curso cadastrado</span>
                                </div>
                        </div>
                </div>
        </div>
                ...
                <form class="p-4" method="POST" enctype="multipart/form-data">
                        <div class="form-row">
                                <div class="form-group col-md-6">
                                        ...
                                        <input type="text" class="form-control" id="titulo" placeholder="Informe o titulo do curso" name="titulo" value="<?= $course["titulo"] ?>">
                                </div>
                                <div class="form-group col-md-6">
                                        <label for="inputEmail4"> Descrição Curta</label>
                                        <input type="text" class="form-control" id="titulo" placeholder="Descrição Curta" name="descricao-curta" value="<?= $course["descricao"] ?>">
                                </div>
                        </div>
                        ...
                        <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="preco" placeholder="R$ 00,00" name="preco-curso" value="<?= $course['preco'] ?>">
                        </div>
                        <div class="form-group">
                                <label for="inputCity">Descrição </label>
                                <textarea type="textarea" cols="40" rows="5" class="form-control" id="inputCity" placeholder="informe a descrição" name="descricao"> <?= $course['conteudo'] ?></textarea>
                        </div>
